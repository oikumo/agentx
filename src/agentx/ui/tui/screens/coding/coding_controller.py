...
class CodingController(ICodingViewPartner):
    """Controller for the Coding screen.
    ...
    def __init__(self, service: CodingAgentService | None = None) -> None:
        """Initialize the controller.

        Args:
            service: An optional injected CodingAgentService. If None, a new
                one is created (which uses AIService().get_current_llm()).
        """
        self._service: CodingAgentService = service or CodingAgentService()
        self._worker_thread: threading.Thread | None = None
        self._app: Any = None  # Set by the View via set_app()
        self._view: Any = None  # Set by the View via set_view()

    # ── ICodingViewPartner implementation ────────────────────────────────────
