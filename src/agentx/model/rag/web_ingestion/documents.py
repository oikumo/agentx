...
async def index_documents_async(vectorstore, documents: List[Document], batch_size: int = 50):
    ...
    print(f"Splitted into {len(batches)} batches of size {batch_size}")
    ...
    async def add_batch(batch: List[Document], batch_num: int):
        try:
            ...
            print(f"Error adding batch, e{e}")
            return False
        return True
    ...
    if successful == len(batches):
        print(f"Documents processed {successful}/{len(batches)}")
    else:
        print(f"Documents processed {successful}/{len(batches)}")
...
def process_documents(result_json_file_path: str) -> list[Document]:
    ...
    print(f"splitted docs: {len(processed_docs)}")
