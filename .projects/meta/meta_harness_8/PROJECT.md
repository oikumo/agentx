...
**Next:** T3-3 resume digest (design note first) / T3-4 task-cost benchmark (anytime) / T3-6 knowledge pilot — 3 remaining; T5-8 fresh-review loop when backlog empties. Scaffold via `new_feature.py "<name>" --type minor_feature --project meta_harness_8`, overlap-check §Decisions D5 first.
...
- [x] T5-1 2A `net transaction authority` DONE 2026-09-13 (feature_079.net_transaction_authority, minor_feature) — `CoordinationLock` + in-lock revision check on every mutation path + `command_id` idempotency; race/idempotency goldens + boundary e2e green, suite 2096 pass.
- [x] T5-2 2B `task claim generation` DONE 2026-09-13 (feature_080.task_claim_generation, minor_feature) — `claim/release/transfer/checkpoint` via `_transact` (owner/session/generation, atomic marking+binding, gen-fenced submit) + CLI ops + plugin whitelist (task_id/owner/generation; budgets 2400→2464/1792→1856, 059 re-pinned); 10 goldens + boundary e2e green, suite 2629 pass (3 pre-existing TUI env failures, stash-verified).
- [x] T5-3 2C `worktree execution isolation` DONE 2026-09-13 (feature_081.worktree_execution_isolation, minor_feature) — new `net/workspace.py` (branch `omt/T/g`, worktree `.worktrees/T-g`, base_commit) + claim/transfer workspace stamp & dir bootstrap + `check_workspace_edit` containment gate (`workspace_mismatch`; stale/owner fenced) + `gate.check_managed_edit_allowed` + CLI `gate --task-id --generation --owner` + claim envelope `task.workspace` (080 pin updated, additive); plugin gate whitelist reuses described args (budgets UNCHANGED 2454/1812); 7 goldens + boundary e2e green, `harnessc check` 0 errors, `build` OK, suite 2113 pass.
- [x] T5-4 2D `two worker capacity scope arbitration` DONE 2026-09-13 (feature_082.two_worker_capacity_scope_arbitration, minor_feature) — code-enforced `worker_slots=2` + component-aware scope arbitration inside `_transact` + capacity-aware probe menu; 7 goldens + boundary e2e green, suite 2120 pass.
- [x] T5-5 3A `verification integration lane` DONE 2026-09-13 (feature_083.verification_integration_lane, minor_feature) — `submit→verify→integrate_start→integrate_finish` via `_transact` (test_slots=1 + integration_slot=1 code-enforced, immutable result refs, coordinator-only verify/integrate, combined-failure returns to pending with evidence); CLI-only (no plugin exposure, budgets unchanged 2454/2464 + 1812/1856); 10 goldens + boundary e2e green, `harnessc check` 0 errors, `build` OK, suite 2130 pass.
- [x] T5-6 3B `recovery and transaction journal` DONE 2026-09-13 (feature_084.recovery_and_transaction_journal, minor_feature) — ship logged in CURRENT_STATE.md; test report @ 6.testing/features/feature_084.recovery_and_transaction_journal/test_report.md.
...
- [x] T2-6 `conventions + lints` DONE 2026-09-13 (feature_089.conventions_and_lints_structural_pin_date_literal, minor_feature) — `harnessc lint` absolute-date-literal advisory subcommand (flags planted literal, clean on `now - timedelta`; NOT folded into check per mh3 scope) + GOTCHA_STRUCTURAL_PIN/GOTCHA_DATE_LITERAL nav-indexed; deliberate re-pins: 059 NAV_INDEX_CEIL 63963→64956 (@budget nav_index 64000→65536), 058 cluster partition 18→20. 5 goldens green; `harnessc check` 0 errors, `build` OK (all budgets green, tool_args 2454/2464); full suite 2173/2173. Discipline TA'd: stage policy_ver chicken-and-egg (edit .omt LAST in a batch, then re-stage).
- [x] T2-7 `scaffolds + LSP allowlist` DONE 2026-09-13 (feature_090.scaffolds_and_lsp_allowlist, minor_feature — mh3 P3-10/P3-11) — `new_feature.py testing/implementation --feature <slug>` scaffold the later-phase artifact paths (exact-match argv pre-scan keeps the legacy positional path byte-identical; unknown-slug/refuse-overwrite rc=2; hermetic PROCESS_ROOT) + `.opencode/lib/enforcer/lsp_filter.ts` (allowlist `.meta/lsp_allowlist.json` seeded with the 9 live pyright-1.1.408 entries: main_controller reportArgumentType×3+reportAttributeAccessIssue×1, tui/provider reportMissingImports×5; severity-1 (file,code) pairs dropped from post-edit text AND metadata, byte-preserving surgery on survivors, fail-open on any surprise incl. desync/WARNING renders) registered in the enforcer after-hook before the raw early-return + HARNESS_FILES coverage. Render contract pinned from live opencode.db samples; fixtures embedded in 25 goldens (recorded shapes A/B + planted-new-error + partial-allowlist byte-preservation + 5 fail-opens + IO probes) + hermetic scaffolder tests + source pins; `harnessc check` 0 errors, `build` OK (nav_index 64956/65536 unchanged); e2e receipt refreshed post-registration; full suite **2198/2198** (2173 + 25 new). Incidental stray repair: feature_089's repo-root `6.testing/.../test_report.md` (the §3.11 defect class itself) git-mv'd to the canonical `.meta/.../6.testing/` path, clearing the root_allowlist hygiene error. Mid-Programming pause + resume exercised (`.sandbox/pause_2026-09-13.md`); feature_090 dogfooded its own scaffolder for impl_notes/test_report.
- [x] T3-7 `budget-diet-bot` DONE 2026-09-13 (feature_091.budget_diet_bot, minor_feature — mh7 P1-4) — harnessc check/build warns (never errors) at ≤64B byte-cap headroom with longest-contributor diet target + `free ≥N B` exit math (tool_args → @tool arg describes, tool_schemas → @tool payload, nav_index → longest nav record; generic trim-or-grow hint otherwise); pure core `budget_diet_warnings` + thin wiring after the past-cap error loop, `measure_budgets`/error loop untouched; 9 goldens green on first run (64/65 boundary, at-cap, over-cap silence, gates/capless/TS-pinned exclusions, sorted multi-budget, lexicographic tie-break, hermetic wiring, live pin). Live-fire: tool_args 2454/2464 (10B) + agents_md 2918/2944 (26B) + tool_schemas 1812/1856 (44B) all warn naming @tool omt_net, nav_index (580B)/ir_json (401B) silent. Staged receipt batch (T4-2 discipline) + boundary e2e green + stage cleared; `harnessc check` 0 errors, `build` OK (no .omt edit, projections byte-identical); suite **2207/2207**; dogfooded feature_090's testing/implementation scaffolders. 6 pre-existing harnessc.py pyright diagnostics verified at HEAD, deliberately NOT allowlisted (static-allowlist tradeoff documented in test report).
- [ ] Next: T3-3 / T3-4 / T3-6 (3 remaining); T5-8 loop when backlog empties. Open repair flag: `meta_harness_development_self_evaluation.md` unindexed (warning) — index into meta_harness META.md or remove, when convenient.
