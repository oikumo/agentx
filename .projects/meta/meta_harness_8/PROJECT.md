...
- [ ] Next: T1-3 `op:graph` transitive risk → T5-1 2A (`net transaction authority`, design note; strict T5 slice order); T3-4 benchmark anytime (no policy change); T5-8 loop when backlog empties. Open repair flag: `meta_harness_development_self_evaluation.md` unindexed (warning) — index into meta_harness META.md or remove, when convenient.
