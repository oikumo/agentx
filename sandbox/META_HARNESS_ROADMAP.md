...
| **M2 — Eliminate repeated context and ceremony** | Selective context delivery, compact tool outputs, stable tool/schema exposure where supported, batched independent reads, a consistent per-operation snapshot, the context assembly order contract (§6.2), and per-tier startup-payload accounting with published fixed-cost ceilings. | M0/M1 measurements identify the main costs. | Lower input tokens per accepted task, fewer redundant calls, and higher cached-input share on warm sessions — all without increased repair, stale decisions or missing obligations. |
