...
class KBPopulator:
    ...
    def _analyze_python_file(self, file_path: Path) -> Dict:
        ...
        for line in lines:
            ...
            if stripped.startswith('class '):
                ...
                if current_class and current_class in class_methods:
                    ...
                    class_methods[current_class] = [func_name]
        ...
        return {
            ...
            'classes': classes,
            'functions': functions,
            'methods_by_class': class_methods,
            'imports': imports[:10],
            'is_source': True,
            'file_type': 'python',
        }
    ...
    def create_kb_entries(self, file_path: Path, analysis: Dict, kb_type: str) -> List[Dict]:
        ...
        if analysis.get('is_source') and analysis.get('file_type') == 'python':
            ...
            # Add entries for each class found with detailed information
            for cls in analysis.get('classes', []):
                class_info = f"Class {cls} in {file_path.name}"
                methods = analysis.get('methods_by_class', {}).get(cls, [])
                solution = f"Class {cls} with methods: {', '.join(methods) if methods else 'No methods'}"
                ...
                # Add individual method entries for each class
                for method in methods:
                    entries.append({
                        "type": "finding",
                        "category": "method",
                        "title": f"Method: {cls}.{method}",
                        "finding": f"Method {method} in class {cls}",
                        "solution": f"Defined in {file_path.name}",
                        "context": f"Class {cls} in {file_path}",
                        "confidence": 0.95,
                        "example": f"{cls}.{method}()"
                    })
            
            # Add individual method entries (standalone functions)
            standalone_functions = [func for func in analysis.get('functions', []) 
                                if not any(func in analysis.get('methods_by_class', {}).get(cls, []) 
                                         for cls in analysis.get('methods_by_class', {}))]
            ...
            for func in standalone_functions:
                entries.append({
                    ...
                    "finding": f"Standalone function in {file_path.name}",
                    "solution": f"Function {func} defined in {file_path.name}",
                    "context": f"Source: {file_path}",
                    "confidence": 0.9,
                    "example": f"def {func}()"
                })
    ...
    def populate(self):
        ...
        for file_path, kb_type in files:
            if self.verbose:
                try:
                    print(f"Processing: {file_path.relative_to(self.base_path.parent)}")
                except ValueError:
...
def main():
    ...
    if args.clean:
        ...
        print("Cleaning database before population...")
        from meta_tools import META_KB_PATH
        import sqlite3
