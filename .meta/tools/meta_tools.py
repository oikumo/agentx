#!/usr/bin/env python3
"""
Meta Tools - Unified Knowledge Base System

Single KB for both Meta Harness patterns and Agent-X project knowledge.
...
Usage:
    ...
    # Add entry
    result = kb.kb_add_entry("pattern", "workflow", "Title", "Finding", "Solution")
...
class KnowledgeBase:
    """Unified knowledge base for Meta Harness and Agent-X."""

    def __init__(self, db_path: Path):
        """Initialize KB with database path.
...
# Single unified KB instance
META_KB_PATH = Path(__file__).parent.parent / ".meta.data" / "kb-meta" / "knowledge-meta.db"
kb = KnowledgeBase(META_KB_PATH)

# Aliases for backward compatibility
meta_kb = kb
agentx_kb = kb  # Now using same KB
...
# Wrapper functions
def kb_search(query: str, top_k: int = 5, category: Optional[str] = None) -> str:
    return kb.kb_search(query, top_k, category)

def kb_ask(question: str, top_k: int = 3) -> str:
    return kb.kb_ask(question, top_k)

def kb_add_entry(entry_type: str, category: str, title: str, finding: str,
                solution: str, context: str = "", confidence: float = 0.5, example: str = "") -> str:
    return kb.kb_add_entry(entry_type, category, title, finding, solution, context, confidence, example)

def kb_correct(entry_id: str, reason: str, new_finding: str) -> str:
    return kb.kb_correct(entry_id, reason, new_finding)
...
def kb_clean_and_populate(kb_name: str = "both", verbose: bool = True) -> str:
    """Clean and populate the unified KB."""
    try:
...
if __name__ == "__main__":
    ...
    print("\nSearch 'TDD':")
    print(kb.kb_search("TDD", top_k=2))

    print("\nAsk 'Where should I write tests?':")
    print(kb.kb_ask("Where should I write tests?")[:500] + "...")
