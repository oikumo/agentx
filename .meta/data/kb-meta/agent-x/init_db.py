#!/usr/bin/env python3
"""
Initialize the Agent-X Knowledge Base database.

This creates a separate KB for Agent-X project knowledge,
distinct from the Meta Harness KB.
"""
...
def init_agent_x_kb(DB_PATH: Path):
    """Initialize the Agent-X knowledge base database.
    ...
    # Create entries table for Agent-X project knowledge
    cursor.execute("""
    ...
    conn.commit()
    conn.close()
    print(f"Agent-X KB initialized at {DB_PATH}")


if __name__ == "__main__":
    # Default path: .meta/data/kb-meta/agent-x/agent-x.db
    KB_PATH = Path(__file__).parent
    DB_PATH = KB_PATH / "agent-x.db"
    init_agent_x_kb(DB_PATH)
