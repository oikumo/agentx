...
def _run_tool(tool_name: str, args: dict | None = None, cwd: Path = REPO_ROOT, timeout: int = 60):
...
def _index_records(root: Path, path_filter: str, kind: str | None = "__none__") -> list[dict]:
...
class TestB2Suggest:
    def test_suggest_ranking_order(self, tmp_path):
        ...
        out = _run_tool("omt_think_suggest", {"path": str(f), "top": 6}, cwd=tmp_path)
    ...
    def test_suggest_top_clamp(self, tmp_path):
        ...
        out = _run_tool("omt_think_suggest", {"path": str(f), "top": 2}, cwd=tmp_path)
        assert out is not None
        assert len(re.findall(r"^\s*\d+\. L\d+", out, re.M)) == 2
        out1 = _run_tool("omt_think_suggest", {"path": str(f), "top": 0}, cwd=tmp_path)
    ...
    def test_suggest_excludes_covered(self, tmp_path):
        ...
        out = _run_tool("omt_think_suggest", {"path": str(f)}, cwd=tmp_path)
        assert out is not None
    ...
    def test_suggest_refuses_protected_and_missing(self, tmp_path):
        ...
        out_p = _run_tool("omt_think_suggest", {"path": "README.md"}, cwd=tmp_path)
        assert out_p is not None and "protected" in out_p
        out_m = _run_tool("omt_think_suggest",
                          {"path": f"no_such_dir_{uuid.uuid4().hex[:6]}/ghost.py"}, cwd=tmp_path)
...
class TestE1Reindex:
    def test_reindex_keep_idempotent(self, tmp_path):
        ...
        rel = f.name
        _seed_index(tmp_path, [{"path": rel, "line": 2, "category": None, "thought": text, "anchor": None}])
        out1 = _run_tool("omt_think_reindex", {"path": rel}, cwd=tmp_path)
        assert out1 is not None
        assert "kept 1" in out1 and "repaired 0" in out1 and "dropped 0" in out1
        out2 = _run_tool("omt_think_reindex", {"path": rel}, cwd=tmp_path)
    ...
    def test_reindex_repairs_drift(self, tmp_path):
        ...
        out = _run_tool("omt_think_reindex", {"path": rel}, cwd=tmp_path)
        assert out is not None
        assert "repaired 1" in out and f"{rel}: 1→3" in out
        adds = _index_records(tmp_path, rel)
        assert len(adds) == 1 and adds[0]["line"] == 3 and adds[0]["repaired_from"] == 1
        assert _index_records(tmp_path, rel, "verify") == [], "old-slot verify must be pruned"
    ...
    def test_reindex_drops_dead_files(self, tmp_path):
        ...
        assert _index_records(tmp_path, rel, None) == []
    ...
    def test_reindex_drops_vanished(self, tmp_path):
        ...
        rel = f.name
        _seed_index(tmp_path, [{"path": rel, "line": 1, "category": None,
    ...
    def test_reindex_ambiguous_drops(self, tmp_path):
        ...
        f = _write_tmp(tmp_path, "amb.py",
                       f"a = 1\n# TA: {text}\nb = 2\n# TA: {text}\n")
        rel = f.name
        _seed_index(tmp_path, [{"path": rel, "line": 1, "category": None, "thought": text, "anchor": None}])
        out = _run_tool("omt_think_reindex", {"path": rel}, cwd=tmp_path)
        assert out is not None
        assert "dropped 1" in out
        assert _index_records(tmp_path, rel, None) == []
