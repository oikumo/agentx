...
def _file_thoughts(abs_path: Path):
...
class TestA1AnchoredPattern:
    ...
    def test_round_trip_every_family(self, tmp_path):
        ...
        for name in ["rt.py", "rt.ts", "rt.md", "rt.css", "rt.sql"]:
            ...
            m = _marker()
            out = _run_tool("omt_think", {"path": str(f), "thought": f"roundtrip {m}"})
    ...
    def test_remove_requires_anchored_line(self, tmp_path):
        ...
        bad = _run_tool("omt_think_remove", {"path": str(f), "line": 1})
        ...
        ok = _run_tool("omt_think_remove", {"path": str(f), "line": 2})
...
class TestA2ExtensionMap:
    ...
    def test_unknown_extension_denied(self, tmp_path):
        ...
        out2 = _run_tool("omt_think", {"path": str(g), "thought": "nope"})
...
class TestA3StringGuard:
    ...
    def test_py_insert_inside_triple_quoted_string_refused(self, tmp_path):
        ...
        before = f.read_text()
        out = _run_tool("omt_think", {"path": str(f), "thought": "inside css", "line": 3})
    ...
    def test_py_insert_after_closed_docstring_allowed(self, tmp_path):
        ...
        out = _run_tool("omt_think", {"path": str(f), "thought": "after css", "line": 4})
    ...
    def test_md_insert_inside_code_fence_refused(self, tmp_path):
        ...
        before = f.read_text()
        out = _run_tool("omt_think", {"path": str(f), "thought": "inside fence", "line": 4})
        assert out is not None and "refused" in out.lower(), f"must refuse: {out!r}"
        assert f.read_text() == before, "file must be unchanged after refusal"
        ok = _run_tool("omt_think", {"path": str(f), "thought": "outside fence", "line": 5})
...
class TestA4FiltersDedupEol:
    def test_category_lowercased_insert_and_filter(self, tmp_path):
        ...
        _run_tool("omt_think", {"path": str(f), "thought": m, "category": "Gotcha"})
        assert f"TA: gotcha: {m}" in f.read_text(), (
            f"category must be lowercased at insert: {f.read_text()!r}")
        out = _run_tool("omt_think_list", {"path": str(tmp_path), "category": "GOTCHA"})
    ...
    def test_query_is_regex_escaped(self, tmp_path):
        ...
        out = _run_tool("omt_think_list", {"path": str(f), "query": "a.b["})
    ...
    def test_duplicate_thought_refused(self, tmp_path):
        ...
        first = _run_tool("omt_think", {"path": str(f), "thought": "mutates history"})
        ...
        dup = _run_tool("omt_think", {"path": str(f), "thought": "mutates history"})
        ...
        ok = _run_tool(
            "omt_think", {"path": str(f), "thought": "mutates history", "category": "gotcha"})
    ...
    def test_crlf_preserved(self, tmp_path):
        ...
        p = tmp_path / "win.py"
        p.write_bytes(b"x = 1\r\ny = 2\r\n")
        out = _run_tool("omt_think", {"path": str(p), "thought": "crlf note"})
